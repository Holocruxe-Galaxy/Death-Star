...
const roleObj: RoleObj = {
  admin: {
    icon: (
      <Box
        component="span"
        sx={{ mr: 2, display: 'flex', color: 'error.main' }}
      >
  ...
  author: {
    icon: (
      <Box
        component="span"
        sx={{ mr: 2, display: 'flex', color: 'warning.main' }}
      >
        <Icon icon="mdi:cog" />
      </Box>
    ),
  ...
  maintainer: {
    icon: (
      <Box
        component="span"
        sx={{ mr: 2, display: 'flex', color: 'success.main' }}
      >
  ...
  subscriber: {
    icon: (
      <Box
        component="span"
        sx={{ mr: 2, display: 'flex', color: 'primary.main' }}
      >
...
const renderUserAvatar = (row: TableBodyRowType) => {
  if (row.avatarSrc) {
    return (
      <CustomAvatar src={row.avatarSrc} sx={{ mr: 3, width: 34, height: 34 }} />
    );
  } else {
    return (
      <CustomAvatar
