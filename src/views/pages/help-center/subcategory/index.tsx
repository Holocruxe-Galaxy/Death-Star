...
const HelpCenterSubcategory = ({ data, activeTab }: Props) => {
  ...
  const handleChange = (event: SyntheticEvent, newValue: string) => {
    setIsLoading(true);
    router
      .push(`/pages/help-center/${data.slug}/${newValue}`)
  ...
  const renderContent = () => {
    const dataToRender = data.subCategories.filter(
      (item: HelpCenterSubcategoriesType) => item.slug === tabValue,
    )[0];
      ...
      <TabPanel value={tabValue} sx={{ p: 0, width: '100%' }}>
        <Card>
          <CardContent>
            ...
            <Box sx={{ mb: 6 }}>
              {dataToRender.articles.map(
                (article: HelpCenterSubcategoryArticlesType) => {
                  return (
                    <Box
                      key={article.title}
                      sx={{
