...
mock.onGet('/apps/email/emails').reply((config) => {
  ...
  const emailsMeta = {
    inbox: data.emails.filter(
      (email: MailType) => !email.isRead && email.folder === 'inbox',
    ).length,
    draft: data.emails.filter((email: MailType) => email.folder === 'draft')
      .length,
    spam: data.emails.filter(
      (email: MailType) => !email.isRead && email.folder === 'spam',
    ).length,
  };
...
mock.onGet('/apps/email/get-email').reply((config) => {
  ...
  if (mail) {
    const mailIndex = paramsFilteredMails.findIndex(
      (i: MailType) => i.id === mail.id,
    );
    mailIndex > 0
...
mock.onGet('/apps/email/paginate-email').reply((config) => {
  ...
  const currentEmailIndex = paramsFilteredMails.findIndex(
    (e: MailType) => e.id === emailId,
  );
  const newEmailIndex =
  ...
  if (newEmail) {
    const mailIndex = paramsFilteredMails.findIndex(
      (i: MailType) => i.id === newEmail.id,
    );
    mailIndex > 0
      ? (newEmail.hasPreviousMail = true)
      : (newEmail.hasPreviousMail = false);
    mailIndex < paramsFilteredMails.length - 1
